...
import { clearDirtyMarker } from "./dirty-markers.js";
...
function withProcessingSignature(fingerprint: SourceFingerprint, signature: string): SourceFingerprint & { processing_signature: string } { return { ...fingerprint, processing_signature: signature }; }
...
export async function scan(config: AppConfig, state: StateDatabase, options: { inventoryOnly?: boolean; force?: boolean; source?: Source; offset?: number; limit?: number } = {}): Promise<ScanSummary> {
  ...
  const summary: ScanSummary = { discovered: 0, queued: 0, unchanged: 0, empty: 0, errors: 0, sourceMissing: 0, excluded: 0, configured: 0, ambiguous: 0, results: [] };
  ...
  for (const adapter of adapters) {
    ...
    try {
      for await (const reference of adapter.discover()) discovered.push({ adapter, reference });
    } catch (error) {
  ...
  for (const item of selected) {
    ...
    if (classification.kind !== "primary") {
      ...
      clearDirtyMarker(config.dirtyDirectory, adapter.source, reference.sourcePath ?? reference.locator);
    ...
    let fingerprint: SourceFingerprint & { processing_signature: string };
    ...
    try {
      ...
      if (!options.inventoryOnly) {
        ...
        clearDirtyMarker(config.dirtyDirectory, adapter.source, session.sourceLocator);
      }
